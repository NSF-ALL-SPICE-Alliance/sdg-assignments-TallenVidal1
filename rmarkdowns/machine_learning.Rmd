...
```
Observations
According to the country_clusters dataframe, each group has been categorized using the country income as an identifier. Group 4 (purple), and Group 1, seem to have the largest quantity of countries, group 4 being High-income countries and group 1 being upper-middle income countries. Using this visualization we could make inferences on how the economic strength of each country influences their normalized score.
...
```{r}
rf_pried <- randomForest(normalized_score_sdg4_primary ~ ., data = sdr_data_imputed, importance = TRUE)
rf_pried
#pried stands for primary (pri) education (ed)#
importance_df_2 <- as.data.frame(rf_pried$importance)
...
ggplot(importance_df_2_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Primary Education Infrastructure",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")

pdp::partial(rf_pried, pred.var = "normalized_score_sdg3_matmort", plot = TRUE)

```

```{r}
pd <- pdp::partial(rf_pried, pred.var = c("normalized_score_sdg3_matmort", "normalized_score_sdg4_second"))
# Default PDP
plotPartial(pd)
```

As the score for sdg 3 maternal mortality and sdg 4 secondary education reaches about 70-75, the model predicts a higher score for the sdg 4 identifier of primary education. Note: The range is only 72-88
